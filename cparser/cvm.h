...
#define LOG_VM 1
    ...
    public:
        ...
        void map_page(uint64_t addr, uint32_t id) override;
        ...
        string_t
        stream_net(vfs_stream_t type, const string_t &path, bool &post, string_t &postfield, bool &bin) override;
        ...
        static void logging(const string_t &s);
    ...
    private:
        ...
        // 虚页映射
        void vmm_map(uint32_t va, uint64_t pa);

        // 解除映射
        void vmm_unmap(uint32_t va);

        // 查询分页情况
        int vmm_ismap(uint32_t va, uint64_t *pa) const;
    ...
    private:
        ...
        struct context_t {
            ...
            std::vector<uint64_t> data_mem;
            std::vector<uint64_t> text_mem;
            std::vector<uint64_t> stack_mem;
            ...
            std::unordered_map<uint32, uint64> pgdir;
            // SYSTEM CALL
