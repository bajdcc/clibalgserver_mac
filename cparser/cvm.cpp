...
#define INC_PTR 4
    ...
    void cvm::logging(const string_t &s) {
    ...
    // 虚页映射
    // va = 虚拟地址  pa = 物理地址
    void cvm::vmm_map(uint32_t va, uint64_t pa) {
        ctx->pgdir.insert(std::make_pair(PAGE_INDEX(va), pa));
#if LOG_SYSTEM
        ATLTRACE("[SYSTEM] MEM  | MAP: PA= %p, VA= %08X\n", (void *) pa, va);
#endif
    }
...
#if LOG_SYSTEM
        ATLTRACE("[SYSTEM] MEM  | UNMAP: VA= %08X\n", va);
#endif
    }

    // 是否已分页
    int cvm::vmm_ismap(uint32_t va, uint64_t *pa) const {
    ...
    T cvm::vmm_get(uint32_t va) const {
        ...
        if (PAGE_INDEX(va) == PAGE_INDEX(va + sizeof(T) - 1)) {
            ...
            auto pid = -1;
            auto succ = true;
            for (auto addr = va; addr < va + (uint32_t) sizeof(T); addr++) {
    ...
    void cvm::exec(int cycle, int &cycles) {
        ...
        for (auto i = 0; i < cycle; ++i) {
...
#if LOG_SYSTEM
                    ATLTRACE("[SYSTEM] ERR  | Invalid PC: %08X\n", ctx->pc);
#endif
            ...
            if (ctx->debug) {
                char a[128], b[128];
                snprintf(a, sizeof(a), "%04d> [%08X] %02d %.4s", i, ctx->pc, op, INS_STRING((ins_t)op).c_str());
                if ((op >= PUSH && op <= LNT) || op == LOAD || op == SAVE)
                    snprintf(b, sizeof(b), " %d", vmm_get(ctx->pc));
                else if (op == IMX)
                    snprintf(b, sizeof(b), " %08X(%d) %08X(%d)", vmm_get(ctx->pc), vmm_get(ctx->pc),
                        vmm_get(ctx->pc + INC_PTR), vmm_get(ctx->pc + INC_PTR));
                else if (op <= ADJ)
                    snprintf(b, sizeof(b), " %08X(%d)", vmm_get(ctx->pc), vmm_get(ctx->pc));
                log << a << b << std::endl;
            }
#endif
            switch (op) {
            ...
            if (ctx->debug) {
                ...
                std::stringstream a;
                char b[128];
                snprintf(b, sizeof(b), "AX: %08X BX: %08X BP: %08X SP: %08X PC: %08X", ctx->ax._u._1, ctx->ax._u._2, ctx->bp, ctx->sp, ctx->pc);
                log << b << std::endl;
                auto k = 0;
                for (uint32_t j = ctx->sp; j < STACK_BASE + PAGE_SIZE; j += 4, ++k) {
                    snprintf(b, sizeof(b), "[%08X]> %08X", j, vmm_get<uint32_t>(j)); a << b;
                    if (k % 4 == 3) {
                        a << std::endl;
                        log << a.str();
                        a.str("");
    ...
    void cvm::destroy(int id) {
        ...
        ctx = tasks[id].get();
        {
            ...
            if (ctx->output_redirect != -1 && tasks[ctx->output_redirect] &&
                tasks[ctx->output_redirect]->flag & CTX_VALID) {
    ...
    void cvm::map_page(uint64_t addr, uint32_t id) {
        uint64_t pa;
    ...
    string_t limit_string(const string_t &s, uint len) {
        if (s.length() <= len) {
            ...
            return s.substr(0, std::max(0, (int) len - 3)) + "...";
        }
    }
        ...
        if (path.substr(0, 6) == "/proc/") {
            ...
            if (std::regex_match(path, res, re)) {
                ...
                if (op == "ps") {
                    std::stringstream ss;
                    ss
                            << "\033FFFA0A0A0\033[STATE] \033S4\033[FLAG] [PID] [PPID]\033FFFB3B920\033 [COMMAND LINE]     \033FFF51C2A8\033[PAGE]\033S4\033"
                            << std::endl;
                    for (auto i = 0; i < TASK_NUM; ++i) {
                        if (tasks[i] && tasks[i]->flag & CTX_VALID) {
                            snprintf(sz, sizeof(sz),
                                     "\033FFFA0A0A0\033%7s \033S4\033 %04X   %4d   %4d \033FFFB3B920\033%-18s \033FFF51C2A8\033  %4lu\033S4\033",
                    ...
                    snprintf(sz, sizeof(sz), "%-18s %d", "Memory Total:", mems * PAGE_SIZE);
                    ss << sz << std::endl;
                    snprintf(sz, sizeof(sz), "%-18s %d", "Heap Total:", heaps * PAGE_SIZE);
                    ss << sz << std::endl;
                    snprintf(sz, sizeof(sz), "%-18s %d", "Heap Using:", heaps * PAGE_SIZE - heaps_a);
                    ss << sz << std::endl;
                    snprintf(sz, sizeof(sz), "%-18s %d", "Heap Free:", heaps_a);
                    ss << sz << std::endl;
                    snprintf(sz, sizeof(sz), "%-18s %d", "Kernel Page:", kernel_pages);
                    ss << sz << std::endl;
                    snprintf(sz, sizeof(sz), "%-18s %d", "User Page:", pages);
                    ss << sz << std::endl;
                    return ss.str();
                } else if (op == "time") {
                    std::stringstream ss;
