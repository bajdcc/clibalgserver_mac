...
namespace clib {
    ...
    void cvfs::ll(const string_t &name, const vfs_node::ref &node, std::ostream &os) const {
        ...
        snprintf(fmt, sizeof(fmt),
                 "\033FFFA0A0A0\033%c%9s \033FFFB3B920\033%4s \033S4\033%9lu \033FFF51C2A8\033%s \033FFF%s\033%s\033S4\033",
                 node->type == fs_dir ? 'd' : '-',
                 (char *) node->mod,
    ...
    void cvfs::load_dir_rec(const string_t &path) {
        struct dirent *dirp;

        DIR *dir = opendir((FILE_ROOT + path + "/").c_str());
