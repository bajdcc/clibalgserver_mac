...
#undef CAST_STR
    ...
    gen_t sym_binop_t::gen_rvalue(igen &gen) {
        switch (op->data._op) {
            ...
            case op_logical_or: {
                ...
                auto t1 = exp1->base->get_cast();
                auto t2 = exp2->base->get_cast();
                if (!(t1 == t_ptr && t2 == t_ptr) && std::max(t1, t2) >= t_long) {
    ...
    int sym_triop_t::size(sym_size_t t, int level) const {
        if (t == x_inc)
            return 0;
        return std::max(std::max(exp1->size(t, level), exp2->size(t, level)), exp3->size(t, level));
    }
    ...
    std::vector<byte> cgen::file() const {
        ...
        // ENTRY ADDR
        bit_write(file, (uint32_t) (std::dynamic_pointer_cast<sym_func_t>(entry->second)->addr));
        // DATA SIZE
        bit_write(file, (uint32_t) (data.size() * sizeof(data[0])));
        // TEXT SIZE
        bit_write(file, (uint32_t) (text.size() * sizeof(text[0])));
        // PDB SIZE
        std::vector<byte> pdb_data;
        {
            bit_write(pdb_data, (uint32_t) pdbs.size());
            auto tmp_idx = (uint32_t) pdb_data.size();
            for (auto &p : pdbs) {
                // INDEX
                bit_write(pdb_data, (uint32_t) std::get<0>(p));
                // ADDR
                bit_write(pdb_data, (uint32_t) 0);
            }
            // CODE
            auto code_idx = (uint32_t) pdb_data.size();
            int i = 0;
            for (auto &p : pdbs) {
                // ADDR
        ...
        bit_write(file, (uint32_t) pdb_data.size());
        // PDB2 SIZE
        std::vector<byte> pdb_data2;
        {
            ...
            bit_write(pdb_data2, (uint32_t) funcs.size());
            auto tmp_idx = (uint32_t) pdb_data2.size();
            for (auto &p : funcs) {
                // INDEX
                bit_write(pdb_data2, (uint32_t) (p->entry - 1));
                // ADDR
                bit_write(pdb_data2, (uint32_t) 0);
            }
            // CODE
            auto code_idx = (uint32_t) pdb_data2.size();
            int i = 0;
            for (auto &p : funcs) {
                // ADDR
                auto addr = (PDB_ADDR *) (pdb_data2.data() + tmp_idx) + i++;
                addr->addr = (uint32_t) (pdb_data2.size()) - code_idx;
                std::stringstream ss;
                string_t _page;
                int L;
                char sz[128];
                if (get_line(p->line, _page, L)) {
                    snprintf(sz, sizeof(sz), "[%s:%d:%d:%d]", _page.c_str(), L, p->column, p->entry - 1);
                } else {
                    snprintf(sz, sizeof(sz), "[???:%d:%d:%d]", p->line, p->column, p->entry - 1);
                }
                ss << sz << " ";
        ...
        bit_write(file, (uint32_t) pdb_data2.size());
        // DATA
        std::copy(data.begin(), data.end(), std::back_inserter(file));
        // TEXT
        std::copy((byte *) text.data(), ((byte *) text.data()) + text.size() * sizeof(text[0]),
            ...
            case c_directDeclarator: {
                if (AST_IS_OP_N(node->child->next, op_lparan)) {
                    ...
                    for (auto &param : func->params) {
                        param->addr += 2 * PTR_SIZE;
                        param->addr_end += 2 * PTR_SIZE;
                    }
#if LOG_TYPE
                        ...
                        if (old_func) {
                            {
                                ...
                                for (size_t i = 0; i < old_func->params.size(); i++) {
                                    if (old_func->params[i]->base->to_string() != func->params[i]->base->to_string()) {
                                        error(asts[0], "conflict declaration in function: " + func->to_string() +
                                                       ", with: " + old_func->to_string() +
                                                       ", wrong param type, need: " +
                                                       old_func->params[i]->to_string() + ", but got: " +
                                                       func->params[i]->to_string());
                                    }
                                }
                            }
                ...
                if (!tmp.back().empty()) {
                    ...
                    for (auto &_t : tmp.back()) {
                        ...
                        if (get_line(_t->line, page, line)) {
                            snprintf(sz, sizeof(sz), "LOC: [%s:%d:%d], CODE: %s", page.c_str(), line, _t->column,
                                     _t->to_string().c_str());
                        } else {
                            snprintf(sz, sizeof(sz), "LOC: [%d:%d], CODE: %s", _t->line, _t->column,
                                     _t->to_string().c_str());
                        }
                        pdbs.push_back(std::make_tuple(text.size(), sz));
                        _t->gen_rvalue(*this);
                    }
                }
            }
                    ...
                    for (auto &exp : list->exps) {
                        if (exp->get_type() == s_unop) {
                            ...
                            if (node->flag != ast_string) {
                                ...
                                std::copy((char *) &node->data._string, (char *) &node->data._string + PTR_SIZE,
                                          std::back_inserter(data));
                            }
                        }
                    }
