...
namespace clib {
    ...
    uint32_t cmem::new_page_single() {
        ...
        auto page_addr = (uint64) memory.back().data();
    ...
    void cmem::copy_from(const cmem &mem) {
        ...
        for (uint64_t i = 0; i < memory.size(); ++i) {
            ...
            auto m1 = memory[i].data();
            auto m2 = mem.memory[i].data();
            memcpy((char *) m1, (char *) m2, PAGE_SIZE);
            memory_page.push_back((uint64_t) m1);
    ...
    void cmem::dump_str(std::ostream &os) const {
        static char sz[200];
        snprintf(sz, sizeof(sz), "PAGE: %lu, FREE: %lu", memory_page.size(), available_size);
        os << sz << std::endl;
        for (auto &f : memory_free) {
    ...
    void cmem::dump() const {
        ...
        ATLTRACE("[SYSTEM] MEM  | PAGE: %lu, FREE: %lu\n", memory_page.size(), available_size);
        for (auto &f : memory_free) {
